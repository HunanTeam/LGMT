    ...
    public partial class ProductController : BasePublicController
    {
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                ExtendInfo1 = product.ExtendInfo1,
                ExtendInfo2 = product.ExtendInfo2,
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        var valueModel = new ProductDetailsModel.ProductAttributeValueModel
                        {
                            ...
                            IsRedirect = attributeValue.IsRedirect,
                            RedirectUrl = attributeValue.RedirectUrl
                        };
        ...
        protected virtual void PrepareProductGroupOverviewModel(ProductGroupOverviewModel model, ProductGroup productGroup)
        {
            ...
            model.ShortDescription = productGroup.ShortDescription;
            model.FullDescription = productGroup.FullDescription;
            model.PictureUrl = _pictureService.GetPictureUrl(productGroup.PictureId);
        }
        ...
        public ActionResult ProductDetails(int productId, int updatecartitemid = 0)
        {
            ...
            /*
            #region 分类
            string cacheKey = string.Format("Nop.pres.category.productdetails-{0}-{1}-{2}",
                    _workContext.WorkingLanguage.Id,
                    string.Join(",", _workContext.CurrentCustomer.GetCustomerRoleIds()),
                    _storeContext.CurrentStore.Id);
        ...
        /// 商品套餐列表
        /// </summary>
        public ActionResult ProductGroups(ProductGroupsModel model, ProductGroupsModel.ProductGroupsFilteringModel command)
        {
            if (model == null)
            ...
            var productGroups = _productGroupService.GetProductGroups(command.PageNumber - 1, command.PageSize, model.tid);
            model.PagingFilteringContext = command;
            model.PagingFilteringContext.LoadPagedList(productGroups);
            model.ProductGroups = productGroups.Select(m =>
            {
                var productGroupOverviewModel = new ProductGroupOverviewModel();
                PrepareProductGroupOverviewModel(productGroupOverviewModel, m);
                return productGroupOverviewModel;
            }).ToList();
        ...
        /// 商品套餐列表
        /// </summary>
        public ActionResult ProductGroup(int id)
        {
            var model = new ProductGroupModel();
            model.ProductGroup = new ProductGroupOverviewModel();

            var productGroup = _productGroupService.GetProductGroupById(id);
            PrepareProductGroupOverviewModel(model.ProductGroup, productGroup);

            var products = _productGroupService.GetProductsByProductGroupId(id)
                .Select(m => m.Product).ToList();
            model.Products = PrepareProductOverviewModels(products).ToList();

            return View(model);
        }

        public ActionResult HomepageProductGroup()
        {
            var productGroup = _productGroupService.GetAllProductGroups()
                .Where(m => m.Recommended).OrderBy(m => m.DisplayOrder).FirstOrDefault();

            ProductGroupOverviewModel productGroupOverviewModel = null;
            if (productGroup != null)
            {
                productGroupOverviewModel = new ProductGroupOverviewModel();
                PrepareProductGroupOverviewModel(productGroupOverviewModel, productGroup);

                var product = _productGroupService.GetProductsByProductGroupId(productGroup.Id)
                    .Select(m => m.Product).OrderBy(m => m.Price).FirstOrDefault();
                if (product != null)
                    ViewBag.MinProductPrice = product.Price;
            }

            return PartialView(productGroupOverviewModel);
        }
