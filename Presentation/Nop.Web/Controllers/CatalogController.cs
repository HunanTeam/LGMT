...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        public ActionResult HomepageProductTags()
        {
            var model = new HomepageProductTagsModel();

            model.ProductTags = _productTagService
                .GetAllProductTags()
            ...
            var products = _productService.GetAllProductsDisplayedOnHomePage(2).ToList();
            model.Products = PrepareProductOverviewModels(products).ToList();
        ...
        public ActionResult Search(SearchModel model, CatalogPagingFilteringModel command)
        {
            ...
            if (model.tid > 0)
            {
                var productTag = _productTagService.GetProductTagById(model.tid);
                if (productTag != null)
                {
                    var productGroup = productTag.ProductGroups.OrderBy(pg => pg.DisplayOrder).FirstOrDefault(pg => pg.Recommended);
                    if (productGroup != null)
                        model.ProductGroup = new ProductGroupOverviewModel
                        {
                            ...
                            PictureUrl = _pictureService.GetPictureUrl(productGroup.PictureId)
                        };
                }
            }
            #endregion
