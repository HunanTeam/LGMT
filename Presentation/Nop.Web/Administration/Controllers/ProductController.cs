...
namespace Nop.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        [NonAction]
        protected virtual void SaveProductTags(ProductGroup productGroup, string[] productTags)
        {
            if (productGroup == null)
                throw new ArgumentNullException("productGroup");

            //product tags
            var existingProductTags = productGroup.ProductTags.ToList();
            var productTagsToRemove = new List<ProductTag>();
            foreach (var existingProductTag in existingProductTags)
            {
                bool found = false;
            ...
            foreach (var productTag in productTagsToRemove)
            {
                productGroup.ProductTags.Remove(productTag);
            ...
            foreach (string productTagName in productTags)
            {
                ...
                if (!productGroup.ProductTagExists(productTag.Id))
                {
                    productGroup.ProductTags.Add(productTag);
                    _productGroupService.UpdateProductGroup(productGroup);
                }
            }
        }
        ...
        /// </summary>
        //[NonAction]
        //protected void SaveCategory(Product product, ProductModel model)
        //{
        //    var category = _categoryService.GetCategoryById(model.CategoryId);
        //    if (category != null)
        //    {
        //        var productCatgory = product.ProductCategories.FirstOrDefault();
        ...
        //                ProductId = product.Id
        //            };
        //            _categoryService.InsertProductCategory(productCatgory);
        //        }
        //        else
        //        {
        //            productCatgory.CategoryId = category.Id;
        //            productCatgory.ProductId = product.Id;
        //            _categoryService.UpdateProductCategory(productCatgory);
        //        }
        //    }
        //}
        ...
        /// 根据商品类别插入默认商品属性
        /// </summary>
        protected void InsertDefaultProductAttributes(Product product, ProductModel model)
        {
            var defaultAttributes = new List<string>();
            if (product.AttributeTemplateId == (int)AttributeTemplate.ChuangLian)//窗帘
                defaultAttributes = new List<string> { "尺寸", "颜色", "房间", "序号", "帘头", "轨道", "窗纱", "遮光布" }; //"窗帘款式",
            else if (product.AttributeTemplateId == (int)AttributeTemplate.GuiDao) //轨道
                defaultAttributes = new List<string> { "轨道款式", "房间", "序号", "颜色", "长度" };
            else if (product.AttributeTemplateId == (int)AttributeTemplate.LuoMaGan) //罗马杆
                defaultAttributes = new List<string> { "罗马杆款式", "房间", "序号", "颜色", "长度" };
            else if (product.AttributeTemplateId == (int)AttributeTemplate.BaiYeChuang) //百叶窗
                defaultAttributes = new List<string> { "百叶窗款式", "房间", "序号", "颜色", "长度", "高度" };

            var productAttributes = _productAttributeService.GetAllProductAttributes().ToList()
                .Where(m => defaultAttributes.Contains(m.Name));

            var displayOrder = 0;
            foreach (var da in defaultAttributes)
            {
                var pa = productAttributes.FirstOrDefault(m => m.Name == da);
                if (pa == null)
                    continue;

                var textPrompt = pa.Name;
                var isRequired = true;
                var attributeControlTypeId = (int)AttributeControlType.DropdownList;
                ...
                var predefinedValues = _productAttributeService.GetPredefinedProductAttributeValues(pa.Id);
        ...
        public ActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //分类
                //SaveCategory(product, model);
                //增加默认商品属性
                InsertDefaultProductAttributes(product, model);
                //discounts
        ...
        public ActionResult ProductTags(DataSourceRequest command)
        {
            ...
            var tags = _productTagService.GetAllProductTags()
                ...
                .Select(x => new ProductTagModel
                {
                    ...
                    Name = x.Name,
                    PictureId = x.PictureId,
                    PictureUrl = _pictureService.GetPictureUrl(x.PictureId, 75),
        ...
        public ActionResult EditProductTag(int id)
        {
            ...
            var model = new ProductTagModel
            {
                ...
                PictureId = productTag.PictureId,
                DisplayOrder = productTag.DisplayOrder,
                Recommended = productTag.Recommended,
        ...
        public ActionResult EditProductTag(string btnId, string formId, ProductTagModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                productTag.DisplayOrder = model.DisplayOrder;
                productTag.Recommended = model.Recommended;
                productTag.PictureId = model.PictureId;
        ...
        public ActionResult ProductAttributeValueCreatePopup(string btnId, string formId, ProductModel.ProductAttributeValueModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    pav.IsRedirect = false;
                    pav.RedirectUrl = null;
                }
                else
                {
                    pav.IsRedirect = model.IsRedirect;
                    pav.RedirectUrl = model.RedirectUrl;
                }
        ...
        public ActionResult ProductAttributeValueEditPopup(int id)
        {
            ...
            var model = new ProductModel.ProductAttributeValueModel
            {
                ...
                IsRedirect = pav.IsRedirect,
                RedirectUrl = pav.RedirectUrl,
        ...
        [HttpPost]
        public ActionResult ProductGroups(DataSourceRequest command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProductTags))
                return AccessDeniedView();
            ...
            var groups = _productGroupService.GetAllProductGroups()
                ...
                .Select(x =>
                {
                    ...
                    productGroupModel.Id = x.Id;
                    productGroupModel.Name = x.Name;
                    productGroupModel.Published = x.Published;
                    productGroupModel.Recommended = x.Recommended;
                    productGroupModel.DisplayOrder = x.DisplayOrder;
                    productGroupModel.ProductCount = 0;
                    productGroupModel.PictureUrl = _pictureService.GetPictureUrl(x.PictureId, 75);
            ...
            var gridModel = new DataSourceResult
            {
                Data = groups.PagedForCommand(command),
                Total = groups.Count
            };
        ...
        public ActionResult ProductGroupDelete(int id)
        {
            ...
            _productGroupService.DeleteProductGroup(group);
        ...
        [HttpPost, ParameterBasedOnFormName("save-continue", "continueEditing")]
        public ActionResult CreateProductGroup(ProductGroupModel model, bool continueEditing)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProductTags))
                return AccessDeniedView();
            ...
            if (ModelState.IsValid)
            {
                ...
                productGroup.PictureId = model.PictureId;
                productGroup.Recommended = model.Recommended;
                productGroup.ShortDescription = model.ShortDescription;
                productGroup.FullDescription = model.FullDescription;
                productGroup.Published = model.Published;
                productGroup.DisplayOrder = model.DisplayOrder;
                productGroup.CreatedOnUtc = DateTime.UtcNow;
                _productGroupService.InsertProductGroup(productGroup);

                //tags
                SaveProductTags(productGroup, ParseProductTags(model.ProductTags));

                return continueEditing ? RedirectToAction("EditProductGroup", new { id = productGroup.Id }) : RedirectToAction("ProductGroups");
            }
        ...
        public ActionResult EditProductGroup(int id)
        {
            ...
            var model = new ProductGroupModel
            {
                ...
                PictureId = productGroup.PictureId,
                Recommended = productGroup.Recommended,
            ...
            {
                var result = new StringBuilder();
                for (int i = 0; i < productGroup.ProductTags.Count; i++)
                {
                    var pt = productGroup.ProductTags.ToList()[i];
                    result.Append(pt.Name);
                    if (i != productGroup.ProductTags.Count - 1)
                        result.Append(", ");
                }
                model.ProductTags = result.ToString();
            }
        ...
        [HttpPost, ParameterBasedOnFormName("save-continue", "continueEditing")]
        public ActionResult EditProductGroup(ProductGroupModel model, bool continueEditing)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProductTags))
                return AccessDeniedView();
        ...
        [HttpPost]
        public ActionResult GroupProductList(DataSourceRequest command, int productGroupId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();

            var productCategories = _productGroupService.GetProductsByProductGroupId(productGroupId,
                    command.Page - 1, command.PageSize, true);
            var gridModel = new DataSourceResult
            {
                Data = productCategories.Select(x => new ProductGroupModel.ProductGroupProductModel
                {
                    Id = x.Id,
        ...
        public ActionResult GroupProductUpdate(CategoryModel.CategoryProductModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
        ...
        public ActionResult GroupProductDelete(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
        ...
        public ActionResult GroupProductAddPopup(int productGroupId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();

            var model = new ProductGroupModel.AddProductGroupProductModel();
            //categories
            model.AvailableCategories.Add(new SelectListItem { Text = _localizationService.GetResource("Admin.Common.All"), Value = "0" });
            var categories = _categoryService.GetAllCategories(showHidden: true);
            foreach (var c in categories)
        ...
        [HttpPost]
        public ActionResult GroupProductAddPopupList(DataSourceRequest command, ProductGroupModel.AddProductGroupProductModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
        ...
        [HttpPost]
        [FormValueRequired("save")]
        public ActionResult GroupProductAddPopup(string btnId, string formId, ProductGroupModel.AddProductGroupProductModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
            ...
            if (model.SelectedProductIds != null)
            {
                var existingProductGroupProducts = _productGroupService.GetProductsByProductGroupId(model.ProductGroupId, showHidden: true);
                ...
                foreach (int id in model.SelectedProductIds)
                {
                    ...
                    {
                        var exist = false;
                        foreach (var productCategory in existingProductGroupProducts)
                            if (productCategory.ProductId == id && productCategory.ProductGroupId == model.ProductGroupId)
                            { exist = true; break; }

                        if (!exist)
                        {
                            _productGroupService.InsertProductGroupProduct(
