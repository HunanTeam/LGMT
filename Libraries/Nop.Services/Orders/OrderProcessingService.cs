...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        /// </summary>
        /// <returns>订单号</returns>
        protected virtual string GenerateOrderNo(int customerId)
        {
            return DateTime.Now.ToString("yyyyMMddHHmmssffff");

            //订单号格式：共13位
            //{年(2位)}{月(2位)}{日(2位)}{店铺Id(4位)}{第几笔单(3位)} //20150925100080

            var date = DateTime.Now.ToString("yyyyMMdd");
            var oCustomerId = customerId.ToString().PadLeft(4, '0');
            var oOrderNo = "01";

            var todayLastOrder = _orderService.SearchOrders(pageSize: 1)
                                .OrderByDescending(m => m.Id).FirstOrDefault();
            if (todayLastOrder != null)
            {
                var no = todayLastOrder.OrderNumber.Substring(11, 2);
                oOrderNo = (int.Parse(no) + 1).ToString().PadLeft(2, '0');
            }

            var orderNo = string.Format("{0}{1}{2}", date, oCustomerId, oOrderNo);
            return orderNo;
        }
            ...
            try
            {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    var order = new Order
                    {
                        ...
                        OrderNumber = GenerateOrderNo(details.Customer.Id),
        ...
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult PlaceStartOrder(ProcessPaymentRequest processPaymentRequest)
        {
            //think about moving functionality of processing recurring orders (after the initial order was placed) to ProcessNextRecurringPayment() method
            if (processPaymentRequest == null)
                throw new ArgumentNullException("processPaymentRequest");
            ...
            try
            {
                ...
                var order = new Order
                {
                    ...
                    AffiliateId = details.AffiliateId,
                    OrderStatus = OrderStatus.Pending,
                    OrderStepId = (int)OrderStep.Pending,
                    ...
                    CustomValuesXml = processPaymentRequest.SerializeCustomValues(),
                    VatNumber = details.VatNumber,
                    CustomerNote = processPaymentRequest.CustomerNote,
                    CreatedOnUtc = DateTime.UtcNow
                };
                _orderService.InsertOrder(order);
