            ...
            foreach (var a2 in attributes2)
            {
                if (a2.IsRequired)
                {
                    ...
                    if (!found)
                    {
                        ...
                        var notFoundWarning = string.Format(_localizationService.GetResource("ShoppingCart.SelectAttribute"), a2.ProductAttribute.GetLocalized(a => a.Name));

                        if (a2.ProductAttribute.Name == "长度")
                            notFoundWarning = "请输入长度";
                        if (a2.ProductAttribute.Name == "高度")
                            notFoundWarning = "请输入高度";

                        warnings.Add(notFoundWarning); 
                        return warnings; //立马返回单个属性错误
                    }
                }
